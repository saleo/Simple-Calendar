...
import net.euse.skcal.models.CalDAVCalendar
...
class EventActivity : net.euse.skcal.activities.SimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(net.euse.skcal.R.layout.activity_event)

        supportActionBar?.setHomeAsUpIndicator(net.euse.skcal.R.drawable.ic_cross)
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        menuInflater.inflate(net.euse.skcal.R.menu.menu_event, menu)
        if (wasActivityInitialized) {
            menu.findItem(net.euse.skcal.R.id.delete).isVisible = mEvent.id != 0
            menu.findItem(net.euse.skcal.R.id.share).isVisible = mEvent.id != 0
        }
        return true
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            net.euse.skcal.R.id.save -> saveEvent()
            net.euse.skcal.R.id.delete -> deleteEvent()
            net.euse.skcal.R.id.share -> shareEvent()
    ...
    private fun setupEditEvent() {
        ...
        supportActionBar?.title = resources.getString(net.euse.skcal.R.string.edit_event)
    ...
    private fun setupNewEvent() {
        ...
        supportActionBar?.title = resources.getString(net.euse.skcal.R.string.new_event)
    ...
    private fun showRepetitionTypePicker() {
        hideKeyboard()
        net.euse.skcal.dialogs.RepeatLimitTypePickerDialog(this, mRepeatLimit, mEventStartDateTime.seconds()) {
            setRepeatLimit(it)
        }
    }
    ...
    private fun checkRepetitionLimitText() {
        event_repetition_limit.text = when {
            mRepeatLimit == 0 -> {
                event_repetition_limit_label.text = getString(net.euse.skcal.R.string.repeat)
                resources.getString(net.euse.skcal.R.string.forever)
            }
            mRepeatLimit > 0 -> {
                event_repetition_limit_label.text = getString(net.euse.skcal.R.string.repeat_till)
                ...
                event_repetition_limit_label.text = getString(net.euse.skcal.R.string.repeat)
                "${-mRepeatLimit} ${getString(net.euse.skcal.R.string.times)}"
            }
        }
    }
    ...
    private fun showRepetitionRuleDialog() {
        hideKeyboard()
        if (mRepeatInterval.isXWeeklyRepetition()) {
            net.euse.skcal.dialogs.RepeatRuleWeeklyDialog(this, mRepeatRule) {
                setRepeatRule(it)
            }
        } else if (mRepeatInterval.isXMonthlyRepetition()) {
            val items = arrayListOf(RadioItem(REPEAT_MONTH_SAME_DAY, getString(net.euse.skcal.R.string.repeat_on_the_same_day)))
            ...
            if (isLastDayOfTheMonth()) {
                items.add(RadioItem(REPEAT_MONTH_LAST_DAY, getString(net.euse.skcal.R.string.repeat_on_the_last_day)))
            }
    ...
    private fun getRepeatXthDayString(includeBase: Boolean, repeatRule: Int): String {
        ...
        return if (includeBase) {
            ...
            val everyString = getString(if (isMaleGender(mEventStartDateTime.dayOfWeek)) net.euse.skcal.R.string.every_m else net.euse.skcal.R.string.every_f)
    ...
    private fun getBaseString(day: Int): String {
        return getString(if (isMaleGender(day)) {
            net.euse.skcal.R.string.repeat_every_m
        } else {
            net.euse.skcal.R.string.repeat_every_f
        })
    }
    ...
    private fun getOrderString(repeatRule: Int): String {
        ...
        return getString(when (order) {
            1 -> if (isMale) net.euse.skcal.R.string.first_m else net.euse.skcal.R.string.first_f
            2 -> if (isMale) net.euse.skcal.R.string.second_m else net.euse.skcal.R.string.second_f
            3 -> if (isMale) net.euse.skcal.R.string.third_m else net.euse.skcal.R.string.third_f
            4 -> if (isMale) net.euse.skcal.R.string.fourth_m else net.euse.skcal.R.string.fourth_f
            else -> if (isMale) net.euse.skcal.R.string.last_m else net.euse.skcal.R.string.last_f
        })
    }

    private fun getDayString(day: Int): String {
        return getString(when (day) {
            1 -> net.euse.skcal.R.string.monday_alt
            2 -> net.euse.skcal.R.string.tuesday_alt
            3 -> net.euse.skcal.R.string.wednesday_alt
            4 -> net.euse.skcal.R.string.thursday_alt
            5 -> net.euse.skcal.R.string.friday_alt
            6 -> net.euse.skcal.R.string.saturday_alt
            else -> net.euse.skcal.R.string.sunday_alt
        })
    }
    ...
    private fun checkRepetitionRuleText() {
        if (mRepeatInterval.isXWeeklyRepetition()) {
            event_repetition_rule.text = if (mRepeatRule == EVERY_DAY) getString(net.euse.skcal.R.string.every_day) else getSelectedDaysString()
        } else if (mRepeatInterval.isXMonthlyRepetition()) {
            val repeatString = if (mRepeatRule == REPEAT_MONTH_ORDER_WEEKDAY_USE_LAST || mRepeatRule == REPEAT_MONTH_ORDER_WEEKDAY)
                net.euse.skcal.R.string.repeat else net.euse.skcal.R.string.repeat_on
    ...
    private fun getSelectedDaysString(): String {
        ...
        if (mRepeatRule and MONDAY != 0)
            days += "${getString(net.euse.skcal.R.string.monday).substringTo(3)}, "
        if (mRepeatRule and TUESDAY != 0)
            days += "${getString(net.euse.skcal.R.string.tuesday).substringTo(3)}, "
        if (mRepeatRule and WEDNESDAY != 0)
            days += "${getString(net.euse.skcal.R.string.wednesday).substringTo(3)}, "
        if (mRepeatRule and THURSDAY != 0)
            days += "${getString(net.euse.skcal.R.string.thursday).substringTo(3)}, "
        if (mRepeatRule and FRIDAY != 0)
            days += "${getString(net.euse.skcal.R.string.friday).substringTo(3)}, "
        if (mRepeatRule and SATURDAY != 0)
            days += "${getString(net.euse.skcal.R.string.saturday).substringTo(3)}, "
        if (mRepeatRule and SUNDAY != 0)
            days += "${getString(net.euse.skcal.R.string.sunday).substringTo(3)}, "

        return days.trim().trimEnd(',')
    }

    private fun getMonthlyRepetitionRuleText() = when (mRepeatRule) {
        REPEAT_MONTH_SAME_DAY -> getString(net.euse.skcal.R.string.the_same_day)
        REPEAT_MONTH_LAST_DAY -> getString(net.euse.skcal.R.string.the_last_day)
        else -> getRepeatXthDayString(false, mRepeatRule)
    }

    private fun showEventTypeDialog() {
        hideKeyboard()
        net.euse.skcal.dialogs.SelectEventTypeDialog(this, mEventTypeId, false) {
            mEventTypeId = it.id
            updateEventType()
        }
    }
    ...
    private fun updateReminder2Text() {
        event_reminder_2.apply {
            ...
            if (mReminder2Minutes == REMINDER_OFF) {
                text = resources.getString(net.euse.skcal.R.string.add_another_reminder)
                alpha = 0.4f
                mReminder3Minutes = REMINDER_OFF
            } else {
    ...
    private fun updateCalDAVCalendar() {
        if (config.caldavSync) {
            ...
            event_caldav_calendar_holder.setOnClickListener {
                hideKeyboard()
                net.euse.skcal.dialogs.SelectEventCalendarDialog(this, calendars, mEventCalendarId) {
    ...
    private fun updateCurrentCalendarInfo(currentCalendar: CalDAVCalendar?) {
        ...
        if (currentCalendar == null) {
            event_caldav_calendar_name.apply {
                text = getString(net.euse.skcal.R.string.store_locally_only)
                setPadding(paddingLeft, paddingTop, paddingRight, resources.getDimension(net.euse.skcal.R.dimen.medium_margin).toInt())
            }
        } else {
            event_caldav_calendar_email.text = currentCalendar.accountName
            event_caldav_calendar_name.apply {
                text = currentCalendar.displayName
                setPadding(paddingLeft, paddingTop, paddingRight, resources.getDimension(net.euse.skcal.R.dimen.tiny_margin).toInt())
            }
        }
    }
    ...
    private fun deleteEvent() {
        net.euse.skcal.dialogs.DeleteEventDialog(this, arrayListOf(mEvent.id)) {
            if (it) {
    ...
    private fun saveEvent() {
        ...
        if (newTitle.isEmpty()) {
            toast(net.euse.skcal.R.string.title_empty)
        ...
        if (newStartTS > newEndTS) {
            toast(net.euse.skcal.R.string.end_before_start)
            return
        }
        ...
        if (mEvent.id == 0) {
            dbHelper.insert(mEvent, true) {
                if (DateTime.now().isAfter(mEventStartDateTime.millis)) {
                    ...
                    toast(net.euse.skcal.R.string.event_added)
                }
            ...
            if (mRepeatInterval > 0 && wasRepeatable) {
                net.euse.skcal.dialogs.EditRepeatingEventDialog(this) {
                    if (it) {
                        ...
                        dbHelper.insert(mEvent, true) {
                            toast(net.euse.skcal.R.string.event_updated)
                            finish()
                        }
                    }
                }
    ...
    private fun checkStartEndValidity() {
        val textColor = if (mEventStartDateTime.isAfter(mEventEndDateTime)) resources.getColor(net.euse.skcal.R.color.red_text) else config.textColor
    ...
    private fun showOnMap() {
        if (event_location.value.isEmpty()) {
            toast(net.euse.skcal.R.string.please_fill_location)
            return
        }
        ...
        if (intent.resolveActivity(packageManager) != null) {
            ...
            toast(net.euse.skcal.R.string.no_app_found)
        }
    }
