...
import net.euse.skcal.helpers.MyWidgetListProvider
...
class WidgetListConfigureActivity : net.euse.skcal.activities.SimpleActivity() {
    ...
    private var mEventsAdapter: net.euse.skcal.adapters.EventListAdapter? = null

    public override fun onCreate(savedInstanceState: Bundle?) {
        useDynamicTheme = false
        super.onCreate(savedInstanceState)
        setResult(Activity.RESULT_CANCELED)
        setContentView(net.euse.skcal.R.layout.widget_config_list)
        ...
        mEventsAdapter = net.euse.skcal.adapters.EventListAdapter(this, getListItems(), false, null, config_events_list) {}
    ...
    private fun getListItems(): ArrayList<ListItem> {
        ...
        listItems.add(ListEvent(1, time.seconds(), time.plusMinutes(30).seconds(), getString(net.euse.skcal.R.string.sample_title_1), getString(net.euse.skcal.R.string.sample_description_1), false, config.primaryColor))
        time = dateTime.withHourOfDay(8)
        listItems.add(ListEvent(2, time.seconds(), time.plusHours(1).seconds(), getString(net.euse.skcal.R.string.sample_title_2), getString(net.euse.skcal.R.string.sample_description_2), false, config.primaryColor))
        ...
        time = dateTime.withHourOfDay(8)
        listItems.add(ListEvent(3, time.seconds(), time.plusHours(1).seconds(), getString(net.euse.skcal.R.string.sample_title_3), "", false, config.primaryColor))
        time = dateTime.withHourOfDay(13)
        listItems.add(ListEvent(4, time.seconds(), time.plusHours(1).seconds(), getString(net.euse.skcal.R.string.sample_title_4), getString(net.euse.skcal.R.string.sample_description_4), false, config.primaryColor))
        time = dateTime.withHourOfDay(18)
        listItems.add(ListEvent(5, time.seconds(), time.plusMinutes(10).seconds(), getString(net.euse.skcal.R.string.sample_title_5), "", false, config.primaryColor))
