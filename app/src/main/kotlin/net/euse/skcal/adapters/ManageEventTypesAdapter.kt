...
class ManageEventTypesAdapter(activity: net.euse.skcal.activities.SimpleActivity, val eventTypes: ArrayList<EventType>, val listener: DeleteEventTypesListener?, recyclerView: MyRecyclerView,
                              itemClick: (Any) -> Unit) : MyRecyclerViewAdapter(activity, recyclerView, null, itemClick) {

    override fun getActionMenuId() = net.euse.skcal.R.menu.cab_event_type
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int) = createViewHolder(net.euse.skcal.R.layout.item_event_type, parent)
    ...
    private fun askConfirmDelete() {
        ...
        if (activity.dbHelper.doEventTypesContainEvent(eventTypes)) {
            ...
            val items = ArrayList<RadioItem>().apply {
                add(RadioItem(MOVE_EVENTS, res.getString(net.euse.skcal.R.string.move_events_into_default)))
                add(RadioItem(DELETE_EVENTS, res.getString(net.euse.skcal.R.string.remove_affected_events)))
            }
    ...
    private fun deleteEventTypes(deleteEvents: Boolean) {
        ...
        for (pos in selectedPositions) {
            if (eventTypes[pos].id == DBHelper.REGULAR_EVENT_TYPE_ID) {
                activity.toast(net.euse.skcal.R.string.cannot_delete_default_type)
