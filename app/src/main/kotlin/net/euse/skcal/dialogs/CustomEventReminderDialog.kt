...
class CustomEventReminderDialog(val activity: Activity, val selectedMinutes: Int = 0, val callback: (minutes: Int) -> Unit) {
    ...
    var view = (activity.layoutInflater.inflate(net.euse.skcal.R.layout.dialog_custom_event_reminder, null) as ViewGroup).apply {
        when {
            selectedMinutes == 0 -> dialog_radio_view.check(net.euse.skcal.R.id.dialog_radio_minutes)
            selectedMinutes % 1440 == 0 -> {
                dialog_radio_view.check(net.euse.skcal.R.id.dialog_radio_days)
                dialog_custom_reminder_value.setText((selectedMinutes / 1440).toString())
            }
            selectedMinutes % 60 == 0 -> {
                dialog_radio_view.check(net.euse.skcal.R.id.dialog_radio_hours)
                dialog_custom_reminder_value.setText((selectedMinutes / 60).toString())
            }
            else -> {
                dialog_radio_view.check(net.euse.skcal.R.id.dialog_radio_minutes)
    ...
    init {
        dialog = AlertDialog.Builder(activity)
                .setPositiveButton(net.euse.skcal.R.string.ok, { dialogInterface, i -> confirmReminder() })
    ...
    private fun getMultiplier(id: Int) = when (id) {
        net.euse.skcal.R.id.dialog_radio_hours -> 60
        net.euse.skcal.R.id.dialog_radio_days -> 1440
        else -> 1
    }
}
