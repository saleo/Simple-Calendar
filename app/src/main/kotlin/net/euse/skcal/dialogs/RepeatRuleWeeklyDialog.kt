...
class RepeatRuleWeeklyDialog(val activity: Activity, val curRepeatRule: Int, val callback: (repeatRule: Int) -> Unit) {
    ...
    val view = activity.layoutInflater.inflate(net.euse.skcal.R.layout.dialog_vertical_linear_layout, null)

    init {
        val days = activity.resources.getStringArray(net.euse.skcal.R.array.week_days)
        val res = activity.resources
        val checkboxes = ArrayList<MyAppCompatCheckbox>(7)
        for (i in 0..6) {
            val pow = Math.pow(2.0, i.toDouble()).toInt()
            (activity.layoutInflater.inflate(net.euse.skcal.R.layout.my_checkbox, null) as MyAppCompatCheckbox).apply {
        ...
        dialog = AlertDialog.Builder(activity)
                .setPositiveButton(net.euse.skcal.R.string.ok, { dialog, which -> callback(getRepeatRuleSum()) })
