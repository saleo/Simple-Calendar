...
class YearFragmentsHolder : MyFragmentHolder() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(net.euse.skcal.R.layout.fragment_years_holder, container, false)
    ...
    private fun setupFragment() {
        ...
        val yearlyAdapter = net.euse.skcal.adapters.MyYearPagerAdapter(activity!!.supportFragmentManager, years)
            ...
            addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
                ...
                override fun onPageSelected(position: Int) {
                    ...
                    if (position < years.size) {
                        (activity as? net.euse.skcal.activities.MainActivity)?.supportActionBar?.title = "${getString(net.euse.skcal.R.string.app_launcher_name)} - ${years[position]}"
                    }
                }
            })
    ...
    override fun refreshEvents() {
        (viewPager?.adapter as? net.euse.skcal.adapters.MyYearPagerAdapter)?.updateCalendars(viewPager?.currentItem ?: 0)
    }

    override fun shouldGoToTodayBeVisible() = currentYear != todayYear

    override fun updateActionBarTitle() {
        (activity as? net.euse.skcal.activities.MainActivity)?.supportActionBar?.title = "${getString(net.euse.skcal.R.string.app_launcher_name)} - $currentYear"
    }
