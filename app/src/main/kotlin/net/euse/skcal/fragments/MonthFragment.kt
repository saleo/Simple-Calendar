...
import net.euse.skcal.extensions.addDayEvents
import net.euse.skcal.extensions.addDayNumber
...
class MonthFragment : Fragment(), MonthlyCalendar {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(net.euse.skcal.R.layout.fragment_month, container, false)
        mRes = resources
    ...
    private fun showMonthDialog() {
        ...
        val view = layoutInflater.inflate(net.euse.skcal.R.layout.date_picker, null)
        val datePicker = view.findViewById<DatePicker>(net.euse.skcal.R.id.date_picker)
        ...
        AlertDialog.Builder(context!!)
                ...
                .setPositiveButton(net.euse.skcal.R.string.ok) { dialog, which -> positivePressed(dateTime, datePicker) }
                .create().apply {
    ...
    private fun updateDays(days: List<DayMonthly>) {
        ...
        for (i in 0 until len) {
            mHolder.findViewById<LinearLayout>(mRes.getIdentifier("day_$i", "id", mPackageName)).apply {
                val day = days[i]
                setOnClickListener {
                    (activity as net.euse.skcal.activities.MainActivity).openDayFromMonthly(Formatter.getDateTimeFromCode(day.code))
                }
