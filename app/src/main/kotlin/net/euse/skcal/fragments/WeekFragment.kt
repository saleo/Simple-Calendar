...
import net.euse.skcal.extensions.getFilteredEvents
import net.euse.skcal.extensions.seconds
import net.euse.skcal.interfaces.WeekFragmentListener
import net.euse.skcal.interfaces.WeeklyCalendar
import net.euse.skcal.models.Event
import net.euse.skcal.views.MyScrollView
...
class WeekFragment : Fragment(), WeeklyCalendar {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mRowHeight = (context!!.resources.getDimension(net.euse.skcal.R.dimen.weekly_view_row_height)).toInt()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        mView = inflater.inflate(net.euse.skcal.R.layout.fragment_week, container, false)
        ...
        (0..6).map { inflater.inflate(net.euse.skcal.R.layout.stroke_vertical_divider, mView.week_vertical_grid_holder) }
        (0..23).map { inflater.inflate(net.euse.skcal.R.layout.stroke_horizontal_divider, mView.week_horizontal_grid_holder) }
    ...
    private fun getDayLetter(pos: Int): String {
        return mRes.getString(when (pos) {
            1 -> net.euse.skcal.R.string.monday_letter
            2 -> net.euse.skcal.R.string.tuesday_letter
            3 -> net.euse.skcal.R.string.wednesday_letter
            4 -> net.euse.skcal.R.string.thursday_letter
            5 -> net.euse.skcal.R.string.friday_letter
            6 -> net.euse.skcal.R.string.saturday_letter
            else -> net.euse.skcal.R.string.sunday_letter
        })
    }
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                if (System.currentTimeMillis() - clickStartTime < CLICK_DURATION_THRESHOLD) {
                    ...
                    val rowHeight = resources.getDimension(net.euse.skcal.R.dimen.weekly_view_row_height)
                    val hour = (event.y / rowHeight).toInt()
                    selectedGrid = (inflater.inflate(net.euse.skcal.R.layout.week_grid_item, null, false) as ImageView).apply {
                        view.addView(this)
    ...
    private fun addEvents() {
        ...
        val fullHeight = mRes.getDimension(net.euse.skcal.R.dimen.weekly_view_events_height)
        val minuteHeight = fullHeight / (24 * 60)
        val minimalHeight = mRes.getDimension(net.euse.skcal.R.dimen.weekly_view_minimal_event_height).toInt()
        ...
        for (event in sorted) {
            if (event.getIsAllDay() || Formatter.getDayCodeFromTS(event.startTS) != Formatter.getDayCodeFromTS(event.endTS)) {
                ...
                (inflater.inflate(net.euse.skcal.R.layout.week_event_marker, null, false) as TextView).apply {
    ...
    private fun addNewLine() {
        val allDaysLine = inflater.inflate(net.euse.skcal.R.layout.all_day_events_holder_line, null, false) as RelativeLayout
    ...
    private fun addCurrentTimeIndicator(minuteHeight: Float) {
        if (todayColumnIndex != -1) {
            ...
            (inflater.inflate(net.euse.skcal.R.layout.week_now_marker, null, false) as ImageView).apply {
                applyColorFilter(primaryColor)
                mView.week_events_holder.addView(this, 0)
                val extraWidth = (todayColumn.width * 0.3).toInt()
                val markerHeight = resources.getDimension(net.euse.skcal.R.dimen.weekly_view_now_height).toInt()
                (layoutParams as RelativeLayout.LayoutParams).apply {
    ...
    private fun addAllDayEvent(event: Event) {
        (inflater.inflate(net.euse.skcal.R.layout.week_all_day_event_marker, null, false) as TextView).apply {
            if (activity == null)
                return
            ...
            (layoutParams as RelativeLayout.LayoutParams).apply {
                topMargin = mRes.getDimension(net.euse.skcal.R.dimen.tiny_margin).toInt()
