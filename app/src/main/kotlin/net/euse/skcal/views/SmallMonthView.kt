...
import net.euse.skcal.helpers.MEDIUM_ALPHA
...
class SmallMonthView(context: Context, attrs: AttributeSet, defStyle: Int) : View(context, attrs, defStyle) {
    ...
    init {
        val attributes = context.theme.obtainStyledAttributes(
                attrs,
                net.euse.skcal.R.styleable.SmallMonthView,
                0, 0)

        try {
            days = attributes.getInt(net.euse.skcal.R.styleable.SmallMonthView_days, 31)
        } finally {
            attributes.recycle()
        }
        ...
        paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = textColor
            textSize = resources.getDimensionPixelSize(net.euse.skcal.R.dimen.year_view_day_text_size).toFloat()
