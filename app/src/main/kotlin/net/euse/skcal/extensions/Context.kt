...
import net.euse.skcal.receivers.CalDAVSyncReceiver
import net.euse.skcal.receivers.NotificationReceiver
import net.euse.skcal.services.SnoozeService
import com.simplemobiletools.commons.extensions.*
import net.euse.skcal.helpers.*
import net.euse.skcal.helpers.Formatter
import net.euse.skcal.models.*
...
fun Context.updateWidgets() {
    ...
    if (widgetsCnt.isNotEmpty()) {
        val ids = intArrayOf(net.euse.skcal.R.xml.widget_monthly_info)
...
fun Context.updateListWidget() {
    ...
    if (widgetsCnt.isNotEmpty()) {
        val ids = intArrayOf(net.euse.skcal.R.xml.widget_list_info)
...
fun Context.getFormattedMinutes(minutes: Int, showBefore: Boolean = true) = when (minutes) {
    -1 -> getString(net.euse.skcal.R.string.no_reminder)
    0 -> getString(net.euse.skcal.R.string.at_start)
    else -> {
        if (minutes % 525600 == 0)
            resources.getQuantityString(net.euse.skcal.R.plurals.years, minutes / 525600, minutes / 525600)

        when {
            minutes % 43200 == 0 -> resources.getQuantityString(net.euse.skcal.R.plurals.months, minutes / 43200, minutes / 43200)
            minutes % 10080 == 0 -> resources.getQuantityString(net.euse.skcal.R.plurals.weeks, minutes / 10080, minutes / 10080)
            minutes % 1440 == 0 -> resources.getQuantityString(net.euse.skcal.R.plurals.days, minutes / 1440, minutes / 1440)
            minutes % 60 == 0 -> {
                val base = if (showBefore) net.euse.skcal.R.plurals.hours_before else net.euse.skcal.R.plurals.by_hours
                resources.getQuantityString(base, minutes / 60, minutes / 60)
            }
            else -> {
                val base = if (showBefore) net.euse.skcal.R.plurals.minutes_before else net.euse.skcal.R.plurals.by_minutes
...
fun Context.getRepetitionText(seconds: Int) = when (seconds) {
    0 -> getString(net.euse.skcal.R.string.no_repetition)
    DAY -> getString(net.euse.skcal.R.string.daily)
    WEEK -> getString(net.euse.skcal.R.string.weekly)
    MONTH -> getString(net.euse.skcal.R.string.monthly)
    YEAR -> getString(net.euse.skcal.R.string.yearly)
    else -> {
        when {
            seconds % YEAR == 0 -> resources.getQuantityString(net.euse.skcal.R.plurals.years, seconds / YEAR, seconds / YEAR)
            seconds % MONTH == 0 -> resources.getQuantityString(net.euse.skcal.R.plurals.months, seconds / MONTH, seconds / MONTH)
            seconds % WEEK == 0 -> resources.getQuantityString(net.euse.skcal.R.plurals.weeks, seconds / WEEK, seconds / WEEK)
            else -> resources.getQuantityString(net.euse.skcal.R.plurals.days, seconds / DAY, seconds / DAY)
        }
    }
}
...
fun Context.notifyEvent(event: Event) {
    ...
    val timeRange = if (event.getIsAllDay()) getString(net.euse.skcal.R.string.all_day) else getFormattedEventTime(startTime, endTime)
...
private fun getNotification(context: Context, pendingIntent: PendingIntent, event: Event, content: String): Notification {
    ...
    if (context.isOreoPlus()) {
        ...
        val name = context.resources.getString(net.euse.skcal.R.string.event_reminders)
    ...
    if (soundUri.scheme == "file") {
        try {
            soundUri = context.getFilePublicUri(File(soundUri.path), net.euse.skcal.BuildConfig.APPLICATION_ID)
    ...
    val builder = NotificationCompat.Builder(context)
            ...
            .setSmallIcon(net.euse.skcal.R.drawable.ic_calendar)
            ...
            .addAction(net.euse.skcal.R.drawable.ic_snooze, context.getString(net.euse.skcal.R.string.snooze), getSnoozePendingIntent(context, event))
...
private fun getPendingIntent(context: Context, event: Event): PendingIntent {
    val intent = Intent(context, net.euse.skcal.activities.EventActivity::class.java)
    intent.putExtra(EVENT_ID, event.id)
    intent.putExtra(EVENT_OCCURRENCE_TS, event.startTS)
...
private fun getSnoozePendingIntent(context: Context, event: Event): PendingIntent {
    val snoozeClass = if (context.config.useSameSnooze) SnoozeService::class.java else net.euse.skcal.activities.SnoozeReminderActivity::class.java
...
fun Context.syncCalDAVCalendars(activity: net.euse.skcal.activities.SimpleActivity?, calDAVSyncObserver: ContentObserver) {
    Thread {
...
fun Context.addDayNumber(rawTextColor: Int, day: DayMonthly, linearLayout: LinearLayout, dayLabelHeight: Int, callback: (Int) -> Unit) {
    ...
    (View.inflate(applicationContext, net.euse.skcal.R.layout.day_monthly_number_view, null) as TextView).apply {
...
private fun addTodaysBackground(textView: TextView, res: Resources, dayLabelHeight: Int, mPrimaryColor: Int) =
        textView.addResizedBackgroundDrawable(res, dayLabelHeight, mPrimaryColor, net.euse.skcal.R.drawable.monthly_today_circle)
...
fun Context.addDayEvents(day: DayMonthly, linearLayout: LinearLayout, res: Resources, dividerMargin: Int) {
    ...
    day.dayEvents.sortedWith(compareBy({ it.startTS }, { it.endTS }, { it.title })).forEach {
        val backgroundDrawable = res.getDrawable(net.euse.skcal.R.drawable.day_monthly_event_background)
        ...
        (View.inflate(applicationContext, net.euse.skcal.R.layout.day_monthly_event_view, null) as TextView).apply {
            setTextColor(textColor)
